...
def load_lit_dataset_sdst(path, read_file=True):
    ...
    wf_instance.ix_job = {str(i): f"job {i}" for i in range(wf_instance.nr_jobs)}
    ...
    for job in range(wf_instance.nr_jobs):
        ...
        while i < len(currentLineValues):
            ...
            wf_instance.ix_operation[str(operation_nr)] = "operation {}".format(
                operation_nr
            )
    ...
    for job in range(wf_instance.nr_jobs):
        for stage in range(1, wf_instance.job_nr_operations[job]):
            preconstr[wf_instance.job_first_operation[job] + stage][
                wf_instance.job_first_operation[job] + stage - 1
            ] = 1
    ...
    wf_instance.ope_clamp_orientation = [0 for _ in range(wf_instance.nr_operations)]
    ...
    wf_instance.ope_night_operation = [1 for _ in range(wf_instance.nr_operations)]
    wf_instance.ope_night_setup = [1 for _ in range(wf_instance.nr_operations)]
    wf_instance.ope_tool_consumption = {
        str(i): {} for i in range(wf_instance.nr_operations)
    }
    wf_instance.ope_tool_similarities = {
        str(i): {} for i in range(wf_instance.nr_operations)
    }
    ...
    wf_instance.sdst = np.zeros(
        shape=(
            wf_instance.nr_machines,
...
if __name__ == "__main__":
    ...
    for instance_path in instances:
        instance: WfInstance = load_lit_dataset_sdst(data_path + instance_path)
        with open(
            cwd + rf"/datasets/research/fjssp-sdst/fattahi-wf/{instance_path}.pickle",
            "wb",
        ) as handle:
