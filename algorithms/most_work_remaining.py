...
def most_work_remaining(instance: WfInstance):
    ...
    schedule = {"allocation": [-1 for _ in range(instance.nr_operations)], "order": []}
    ...
    for _ in range(instance.nr_operations):
        ...
        scheduled_op = (
            instance.job_first_operation[job_nr] + job_ope_counter[job_nr] - 1
        )
        scheduled[scheduled_op] = True

        temp_total_load = [
            total_load[j] if instance.duration[scheduled_op][j] > 0 else np.inf
            for j in range(instance.nr_machines)
        ]
        selected_machine = np.argmin(temp_total_load)
        total_load[selected_machine] += instance.duration[scheduled_op][
            selected_machine
        ]
